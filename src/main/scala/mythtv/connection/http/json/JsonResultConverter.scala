...
object JsonResultConverter extends ServiceResultConverter with FrontendJsonProtocol with BackendJsonProtocol {
  ...
  def listRecRuleFilter(r: RN): List[RecRuleFilter]           = value(r).convertTo[List[RecRuleFilter]]
  ...
  def pagedListRecRuleFilter(r: RN): PL[RecRuleFilter]        = value(r).convertTo[ServicesPagedList[RecRuleFilter]]
