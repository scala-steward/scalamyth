...
abstract class JsonService(conn: JsonConnection)
  ...
  def responseRoot(response: JsonResponse, level0Field: String, level1Field: String): Try[JsValue] =
    Try(response.json.asJsObject.fields(level0Field).asJsObject.fields(level1Field))
