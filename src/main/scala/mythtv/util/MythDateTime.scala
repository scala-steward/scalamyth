...
object MythDateTime {
  ...
  def fromIso(isoDt: String): MythDateTime = new MythDateTime(Instant.parse(isoDt))
  ...
  def fromTimestamp(ts: Long): MythDateTime = new MythDateTime(Instant.ofEpochSecond(ts))

  def now: MythDateTime = new MythDateTime(Instant.now)
